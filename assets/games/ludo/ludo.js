...
function HaveHover() {                            //  Attacking Pawn
    var count = 0;
    var toKill = "";
...
}// A loop to change player if 6 does not appear
...
function DontHaveOtherFree() {
    ...
    return true;
}//If all paws are out, winner is declared
...
function ResetPawn(victim) {        //If pawn is killed by other pawn, killed pawn shift to 0 position
...
function randomNum() {//If dice is not clicked, dice move continuously until clicked
    if (!clicked) {
        num = Math.floor((Math.random() * 6) + 1);;
        var dice = document.getElementById('dice');
        dice.style.backgroundImage = "url(image/" + num + ".jpg)";
        clicked = true;
    }// For those paws which dont come out and 6 is not coming (STUCK)
...
}// For that paw which is going to enter Box
